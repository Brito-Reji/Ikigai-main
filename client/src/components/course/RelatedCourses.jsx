...
const RelatedCourses = ({ currentCourse, category }) => {
  ...
  useEffect(() => {
    const fetchRelatedCourses = async () => {
      ...
      try {
        setLoading(true);
        const response = await api.get(`/public/courses?category=${currentCourse.category._id}&limit=4`);
        
        if (response.data.success) {
          // Filter out the current course
          const filtered = response.data.data.filter(course => course._id !== currentCourse._id);
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">More Courses Like This</h2>
