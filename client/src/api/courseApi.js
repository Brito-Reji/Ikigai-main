...
export const courseApi = {
    instructor: {
        ...
        createCourse: async (courseData) => {
            const { data } = await api.post(courseEndpoints.instructor.create(), courseData)
            return data
        },

        updateCourse: async ({ courseId, courseData }) => {
            const { data } = await api.put(courseEndpoints.instructor.update(courseId), courseData)
            return data
        },

        applyForVerification: async (courseId) => {
            const { data } = await api.post(courseEndpoints.instructor.applyVerification(courseId))
            return data
        },

        togglePublish: async (courseId) => {
            const { data } = await api.patch(courseEndpoints.instructor.togglePublish(courseId))
            return data
        },
    },
    ...
    student: {
        getCourses: async (params) => {
            const { data } = await api.get(courseEndpoints.student.list(), { params })
            return data
        },
    ...
    public: {
        ...
        getFeaturedCourses: async (params) => {
            const { data } = await api.get(courseEndpoints.public.featured(), { params })
            return data
        },
    ...
    admin: {
        getCourses: async (filters = {}) => {
            ...
            const { data } = await api.get(`${courseEndpoints.admin.list()}?${params.toString()}`)
            return data
        },
        ...
        toggleBlock: async (courseId) => {
            const { data } = await api.patch(courseEndpoints.admin.toggleBlock(courseId))
            return data
        },
        ...
        updateVerification: async ({ courseId, status, rejectionReason }) => {
            const { data } = await api.patch(courseEndpoints.admin.updateVerification(courseId), {
                status,
                rejectionReason,
            })
            return data
        ...
        getPendingVerifications: async (page = 1, limit = 20) => {
            const { data } = await api.get(`${courseEndpoints.admin.pendingVerifications()}?page=${page}&limit=${limit}`)
            return data
        },

        getVerificationStatistics: async () => {
            const { data } = await api.get(courseEndpoints.admin.verificationStatistics())
            return data
        },
    },
}
