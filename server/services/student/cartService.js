...
export const getCartService = async userId => {
  ...
  // filter nulls
  const filteredCourses = (cart?.courses || []).filter(
    course => course !== null && course.category && !course.category.isBlocked
  );
  ...
  if (cart && filteredCourses.length !== cart.courses.length) {
    ...
    await Cart.findOneAndUpdate({ userId }, { $set: { courses: validIds } });
  }
